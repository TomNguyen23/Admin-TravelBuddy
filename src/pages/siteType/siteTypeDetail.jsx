	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			<DialogTrigger asChild>
				<Button variant="outline" onClick={() => setOpen(true)}>Thêm khía cạnh</Button>
			</DialogTrigger>
			<DialogContent className="sm:max-w-md">
				...
				<div className="flex items-center space-x-2">
					<div className="grid flex-1 gap-2">
						...
						<Input id="aspectName" defaultValue="" placeholder="Tên khía cạnh"/>
					</div>
					<Button type="submit" size="sm" className="px-3" onClick={onAdd}>
	...
	const fetchAspectsByType = async () => {
		try {
			...
			response.data.map(aspect => {
				...
				setNewAspects(...newAspects, aspect.aspectName)
			})
			setAspects(aspects)
		} catch (error) {
			console.log(error);
		...
		<div className="bg-white border shadow-lg rounded-md text-2xl text-center relative right-0 top-[-8rem] left-0 mx-12 my-7 px-5 pb-10 min-h-[75%]">
			...
			<div className="">
				...
				<div>
					...
					<AspectDialog open={aspectOpen} setOpen={setAspectOpen} onAdd={() => {}} />
				</div>
			</div>
		</div>
	);
